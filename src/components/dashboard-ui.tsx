  ...
  return (
    <DialogPrimitive.Root>
      ...
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay className="fixed inset-0 z-40 bg-ink/40 backdrop-blur-[2px] data-[state=open]:animate-in" />
        <DialogPrimitive.Content className="fixed left-1/2 top-1/2 z-50 w-[92vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-line bg-surface p-5 shadow-2xl data-[state=open]:animate-in">
  ...
  return (
    <DropdownPrimitive.Root>
      <DropdownPrimitive.Trigger asChild>
        <button className="relative grid h-10 w-10 place-items-center rounded-xl border border-line bg-surface text-muted hover:bg-surface-2 hover:text-ink" aria-label="Notifications">
          <Bell size={18} />
          {unread > 0 && (
            <span className="absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full bg-rose px-1 text-[10px] font-semibold text-white">{unread}</span>
          )}
        </button>
      </DropdownPrimitive.Trigger>
      <DropdownPrimitive.Portal>
        <DropdownPrimitive.Content align="end" sideOffset={8} className="z-50 w-80 rounded-2xl border border-line bg-surface p-2 shadow-2xl data-[state=open]:animate-in">
...
export function UserMenu({ name, orgName, logoutAction }: { name: string; orgName: string; logoutAction: () => Promise<void> }) {
  return (
    <DropdownPrimitive.Root>
      <DropdownPrimitive.Trigger asChild>
        <button className="flex items-center gap-2.5 rounded-xl border border-line bg-surface py-1.5 pl-1.5 pr-3 hover:bg-surface-2">
      ...
      <DropdownPrimitive.Portal>
        <DropdownPrimitive.Content align="end" sideOffset={8} className="z-50 w-52 rounded-2xl border border-line bg-surface p-1.5 shadow-2xl data-[state=open]:animate-in">
