...
export type SettleResult = {
   ...
   *  using this payload. */
  auditDetail?: { tenantId: string; invoiceNumber: string; fromPlan: string; toPlan: string; remainingValueKes: number; payableKes: number; paidTotalKes: number; connectorAllowanceChange: { from: number | null; to: number | null } | null; messageTopupMessages: number; messageTopupKes: number; aiTopupKes: number };
};
  ...
  if (result.outcome === "settled" && result.auditDetail) {
    await audit({
      ...
      detail: {
        ...
        messageTopupMessages: result.auditDetail.messageTopupMessages, messageTopupKes: result.auditDetail.messageTopupKes, aiTopupKes: result.auditDetail.aiTopupKes,
      },
    }).catch(() => {});
  }
...
async function settleInvoiceInTx(tx: Tx, invoiceId: string, expiryHours: number): Promise<SettleResult> {
  ...
  await tx.subscription.update({
    ...
    data: {
      ...
      // Real message + AI-understanding balance top-up bundled into this
      // same payment, 2026-08-27 — credited only now, once payment has
      // actually settled, same "money moves only when it's real" rule the
      // plan change itself follows. Both balances funded together (a real
      // reply always draws from both) — a no-op increment(0) when no
      // top-up was purchased.
      ...(before.messageTopupKes > 0 ? { messageBalanceKes: { increment: before.messageTopupKes } } : {}),
      ...(before.aiTopupKes > 0 ? { aiBalanceKes: { increment: before.aiTopupKes } } : {}),
    },
  });
  ...
  return {
    ...
    auditDetail: {
      ...
      messageTopupMessages: before.messageTopupMessages, messageTopupKes: before.messageTopupKes, aiTopupKes: before.aiTopupKes,
    },
  };
}
