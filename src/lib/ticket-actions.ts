...
// Explicit `ok` discriminant (rather than relying on "error" in found)
// so this narrows soundly via TypeScript's discriminated-union analysis —
// an ad hoc union inferred purely from two differently-shaped `as const`
// returns does not always narrow reliably via property-presence checks.
...
export async function assignTicketAction(ticketId: string, assignedAdminId: string) {
  ...
  return withAssertAdminPermission("tickets.manage", async (admin) => {
    ...
    return { ok: true };
  }, { tenantId: found.ticket.tenantId });
}
...
export async function addInternalNoteAction(ticketId: string, body: string) {
  ...
  const found = await loadTicketOrError(ticketId);
  if (!found.ok) return { error: found.error };
  return withAssertAdminPermission("tickets.internal_notes", async (admin) => {
...
export async function linkIncidentAction(ticketId: string, incidentIdOrNumber: string): Promise<{ error: string } | { ok: true }> {
...
export async function resolveTicketAction(ticketId: string, resolution: string, resolutionReason: string) {
  ...
  return withAssertAdminPermission("tickets.resolve", async (admin) => {
