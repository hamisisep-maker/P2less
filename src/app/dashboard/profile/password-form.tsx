  ...
  return (
    <form ref={formRef} action={action} className="max-w-sm space-y-3">
      <label className="block">
        ...
        <PasswordInput name="currentPassword" required className="mt-1 w-full rounded-xl border border-line bg-surface px-3 py-2 text-sm outline-none focus:border-accent" />
      </label>
      <label className="block">
        <span className="text-xs font-medium text-muted">New password</span>
        <PasswordInput name="newPassword" required minLength={8} className="mt-1 w-full rounded-xl border border-line bg-surface px-3 py-2 text-sm outline-none focus:border-accent" />
