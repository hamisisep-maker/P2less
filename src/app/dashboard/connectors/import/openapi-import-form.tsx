...
export function OpenApiImportForm() {
  ...
  function handleParse() {
    ...
    setParseError(null);
    setConnectorName(result.suggestedName);
    setConnectorDescription(result.suggestedDescription);
    setBaseUrl(result.suggestedBaseUrl);
    setActions(result.actions.map((a) => ({ ...a, included: true, requiredPermission: "" })));
    setStep("review");
  }
    ...
    <form action={createAction} className="space-y-4">
      ...
      <Card className="space-y-4 p-5">
        ...
        <div>
          ...
          <p className="mt-1 text-[11px] text-faint">Credentials are encrypted at rest and never shown again or logged. An OpenAPI spec rarely contains a real secret — fill this in yourself.</p>
        </div>
        {authType === "api_key" && (
          <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="flex items-center gap-3">
        <button type="button" onClick={() => setStep("paste")} className="rounded-xl border border-line px-5 py-2.5 text-sm font-medium text-muted hover:bg-surface-2">
          Back to spec
        </button>
        <button
          type="submit"
