...
*Last updated: 2026-08-25 (production Stripe webhook endpoint registered and live-verified with a genuinely Stripe-delivered signed event — Card itself stays gated pending a live-mode Stripe key). Update this doc (and push it) in the same round as any change that ships, closes an external item, or surfaces a new gap — don't let it go stale.*
