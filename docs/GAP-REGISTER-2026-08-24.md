...
1. **Tenant-scoping ORM extension fails OPEN, not closed, when no tenant context is set — root cause now correctly diagnosed and fixed; `db.ts` deliberately still warn-only for one more deploy cycle before the final flip.** The original 2026-08-23 write-up blamed "React Server Component page renders don't propagate AsyncLocalStorage" — that theory was wrong. 2026-08-24: reproduced the real failure directly against a genuine `next build && next start` (the prior fix was only ever verified under `next dev`, which hides this): the actual mechanism is that `enterTenantContext()`/`enterCrossTenantContext()` called INSIDE a nested async guard function (`requireTenantUser()`, `assertAdminPermission()`) and then RETURNING a value for the caller to keep using does not survive — the context is lost the instant that function returns, even one line later, even in Route Handlers (not RSC-specific at all). What reliably survives, proven the same way: set context, then synchronously invoke a callback in the same frame — even across further awaits, even across module boundaries, even read back from db.ts's own extension.
   ...
   `db.ts` itself is being left on warn-only for one more production deploy cycle on purpose — watch `railway logs` for `[TENANT-CONTEXT-MISSING]` on real traffic; if it stays silent, flip to fail-closed in a separate, later deploy. Given this exact area caused one real incident already, the flip itself is being treated as its own decision point, not bundled into this fix.
...
The most consequential item on this whole list, **the tenant-scoping fail-open default** (security section, item 1), had its real root cause found and fixed 2026-08-24 — `db.ts` is deliberately still warn-only pending one clean deploy cycle before the final flip to fail-closed, tracked as its own decision. Until Meta's review clears, **Business Verification** (item 2, now submitted and pending) is the other item with real external dependency. Everything else still open is real but lower-severity — either a genuine, deliberately-scoped-later feature gap (billing self-service, a few dashboard views), or a deliberate product decision correctly not mistaken for a bug.
