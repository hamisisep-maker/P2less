...
### 6. CRITICAL, stopgap shipped 2026-08-25 — self-service upgrade activated with ZERO payment, real live money exposure
...
- **Confirmed by re-reading the actual code** (`upgradeSubscriptionPlanAction`, `src/lib/actions.ts`): a bare `db.subscription.update({ data: { planId } })`. No `computeBill()` call, no `Payment` row created, no `stkPush()`, no Stripe charge, no confirmation gate — the click itself moves the tenant to the higher tier for free, immediately, permanently, until they downgrade or cancel. The "Upgrades apply immediately" design decided 2026-08-25 (item 4 above) always meant "immediately once paid" in intent, but the payment half of that was never actually built — a real gap in that shipped feature, not a new regression.
- **Stopgap shipped same day**: self-service upgrade now always returns "Self-service upgrade is temporarily unavailable while we finish a secure payment flow for it. Contact us and we'll upgrade your plan right away." — same "admin-only, contact us" shape already used for downgrades. Admin-side `changeTenantPlanAction` (admin-actions.ts) is deliberately left untouched — that path requires a human with permission and a recorded reason, a legitimate manual override, not a self-service bypass. Verified via full regression suite (74/74) and typecheck.
- **Real design in progress, invoice-centric**: the user specified a complete invoice-centric payment architecture (invoice as the primary billing reference, not phone/account/provider-transaction id; Visa/card + Paybill + STK Push all converging on one invoice; backend-computed amounts never trusted from the client; proration of remaining plan value on upgrade; usage/connector preservation; full audit trail; idempotent, non-double-chargeable, crash-safe end to end). This is a substantial architecture, not a button fix — being designed via a full plan before implementation, not built ad hoc.
