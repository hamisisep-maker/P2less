...
- **Live-verified end to end**, not just typechecked: real tenant login (Kilimani Retail), 15-days-into-a-30-day-cycle upgrade Starter→Professional — invoice showed exact proration (`remaining KES 750`, `payable KES 4,150`), real Safaricom Daraja sandbox STK push fired, simulated callback settled the invoice, subscription plan changed, `messageBalanceKes`/`aiBalanceKes`/connectors all confirmed untouched, full `invoice.created → invoice.settled` audit trail. Replayed the identical callback — confirmed exactly one settlement, no double-apply. Constructed and verified the zero-payable edge case (temporarily inflated a plan price) — real `KES 0` invoice went through the identical `settleInvoice()` path, not a bypass. Pure proration math additionally verified against real Feb 28/29-day months, 30/31-day months, past-renewal clamping, and the trial-tenant zero-credit case.
- **Still open, tracked as separate later stages, not silently dropped**: Paybill (needs new invoice-number-based matching — today's Paybill/C2B confirmation matches a fixed one-per-subscription reference, not per-invoice) and Visa/Card via Stripe (today only `$0` SetupIntent card-verification exists — zero PaymentIntent/Checkout/webhook code anywhere, a genuinely new subsystem) both show as visibly disabled "Coming soon" in the payment method modal, never hidden or faked as available. The broader app-wide sweep replacing `window.confirm()`/toast-only destructive-action confirmations with the new `src/components/modal.tsx` (built this round, used so far only for the payment modal) is also a separate future stage.
