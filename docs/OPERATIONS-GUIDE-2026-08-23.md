...
**B. A platform-level operating-mode / feature-flag system — registration, onboarding, and login as controllable switches, with a full audit trail.** Proposed as a 3-tier system: a system-wide operating mode (PRE-LAUNCH / PRODUCTION / MAINTENANCE), per-feature controls (registration, onboarding, login, each channel) respected by both UI and backend, and per-training-session controls — plus required-reason confirmation dialogs and a full audit log of every toggle. **Grounded check, not hypothetical**: this risk is real today, not abstract — `/onboard` (`src/app/onboard/page.tsx`) is a live, public, self-serve tenant-creation route; anyone with the link can create a tenant right now, hardened against trial abuse (roadmap doc's 5 trial-abuse items) but with no master off switch. **Recommendation given, not yet built**: the full 12-part system (three operating modes, three tiers of controls, per-channel kill switches, mandatory reason fields, a dedicated audit UI) is real infrastructure for a team running change control over many toggles — premature for one founder pre-launch. The single piece worth building when needed is one backend-enforced flag, `Public tenant registration: ON/OFF`, checked inside the `/onboard` server action itself (not just hidden in the UI — a direct call must be blocked too), toggle-able by super admin only, defaulting to **OFF** given today's zero-real-client state. Not built this round; recorded here so the actual current exposure (a live public tenant-creation route with no off switch) isn't lost track of.
