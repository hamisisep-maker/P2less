...
### Real request tampering — authorized self-pentest of the app's own server-side validation

Everything below is done against your own production system, with your own authorization — legitimate security testing, not an attack on anything external. "Burp Suite" isn't literally needed: intercepting and altering a request before the server sees it is done here via the browser's own dev tools / direct scripted requests carrying deliberately manipulated payloads, which achieves the identical result. The whole point is verifying the discipline already stated in this codebase's own comments — "the client never submits an amount, re-verified server-side" — is actually true everywhere it's claimed, not just where it's convenient.

7. **Tamper with dates client-side.** `Subscription.trialEndsAt`, the invoice's `remainingDays`/`daysInCycle`, any date the UI merely *displays* — try editing them via browser dev tools (React state, or direct DOM/localStorage edits) and reloading, or replaying an old page state. Expected: cosmetic only, changes nothing server-side, because the real value always gets re-read from the DB on the next real request. Confirm this is actually true, not assumed.
8. **Tamper with the plan/price in-flight.** Intercept the real request `UpgradeModal` sends to `createUpgradeInvoiceAction` (or call the underlying Server Action directly with a crafted payload) and submit a real Business/Enterprise `planId` alongside a manipulated `extraMessages` (negative, absurdly large, or non-integer) or an attempt to pass a client-supplied price/amount directly. Expected: the action re-computes everything from real DB state and the real `price_conversation_kes`/`price_ai_kes` settings regardless of what was submitted — confirm no path exists where a submitted number reaches `payableKes` unchecked.
9. **Tamper with the payment amount itself.** For a real STK/Paybill/card attempt, try submitting a payment amount that's genuinely different from what the invoice actually says is owed (over or under). Expected: `settleInvoiceInTx`'s own sum-of-paid-payments-vs-`payableKes` check is the only thing that ever triggers settlement — confirm an underpayment never silently settles, and an overpayment never grants anything beyond what was actually invoiced.
10. **Tamper with identity/session.** Try modifying the session cookie's tenant reference (or replaying Tenant 5's cookie against a URL/ID belonging to Tenant 1–4), try calling a Server Action meant for one tenant's data while authenticated as another, try escalating a regular tenant session into an admin one by altering whatever claims/cookie the session carries. Expected: `withTenantUser`'s tenant-scoping and the Prisma extension's fail-closed `TenantContextMissingError` behavior (the same mechanism already found and fixed once this session, per GAP-REGISTER's security section item 1) should make every one of these a hard failure — this is the single highest-value thing to actually try, given a real historical bug in exactly this area.
11. **General tamper-and-observe.** With the browser's network tools open, walk through the whole signup → upgrade → pay → use flow once more, and for every real request fired, ask "what happens if I resend this exact request again," "what happens if I strip a field," "what happens if I change an ID in the URL to one that isn't mine." Log every single thing tried, whether it succeeded or correctly failed — this general pass is what actually surfaces the gaps the structured list above doesn't anticipate.
